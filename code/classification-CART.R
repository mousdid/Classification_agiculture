...
validation_kfold <- function(data_train, k) {
  ...
  for (i in 1:k) {
   ...
   #modele
     tree <- rpart( ensemble_apprentissage_y~., data = ensemble_apprentissage_x)
    print(tree)
    summary(tree)
    ...
    #data_test_predict <- predict(tree, newdata=ensemble_test_x, type="class")
    data_test_predict <- predict(tree, newdata=ensemble_test_x, type="class")
    data_test_predict_with_proba <- predict(tree, newdata=ensemble_test_x, type="prob")
    seuil <-0.4 #comme 0.5 comme 0.6
    ...
    data_test_predict_with_proba <- predict(tree, newdata=ensemble_test_x, type="class")
    data_test_predict_with_proba_proba <- predict(tree, newdata=ensemble_test_x, type="prob")
    ...
    pred_cart <- prediction(score, ensemble_test$DIFF)
    perf <- performance(pred_cart, "tpr", "fpr")
    plot(perf,colorize=TRUE)
    par(new=T)
    plot(c(0,1),c(0,1),type="l",ann=FALSE)
    
    # Aire sous la courbe
    AUC[i] <- performance(pred_cart, "auc")@y.values[[1]]
    cat("AUC = ", AUC[i],"\n")
