...
validation_kfold <- function(data_train, k) {
  ...
  for (i in 1:k) {
    ...
    # Bayesien
    naiv_bayes <- naiveBayes(DIFF~., data = data_train)
    
    # Prédiction sur l'ensemble de test
    data_test_predict <- predict( naiv_bayes, newdata=ensemble_test_x, type="class")
    data_test_predict_with_proba <- predict( naiv_bayes, newdata=ensemble_test_x, type="raw")
    seuil <-0.4 #comme 0.3
  ...
  # print (data_test_predict)
   #print (ensemble_test$DIFF)
    ...
    score <- data_test_predict_with_proba[,2]
