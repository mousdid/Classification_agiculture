...
validation_kfold <- function(data_train, k) {
  ...
  for (i in 1:k) {
    ...
    # LDA
    lin_disc_an <- lda(DIFF~., data = ensemble_apprentissage)
    
    # Prédiction sur l'ensemble de test
    data_test_predict <- predict(lin_disc_an, newdata=ensemble_test_x, type="class")
    data_test_predict_with_proba <- predict(lin_disc_an, newdata=ensemble_test_x, type="prob")
    seuil <-0.4
    data_test_predict$class <- ifelse( data_test_predict_with_proba$posterior[,2] > seuil,"1","0")
    ...
    error_rate[i] <- mean(data_test_predict$class != ensemble_test$DIFF)
    
    cat("error_rate using test data = ",error_rate[i],"\n")
    
    # Matrice de confusion
    confmat = table(data_test_predict$class,ensemble_test$DIFF)
    ...
    data_test_predict_with_proba <- predict(lin_disc_an, newdata=ensemble_test_x, type="prob")
    print(data_test_predict_with_proba$posterior)
    
    
    
    # Calcul du score
    score <- data_test_predict_with_proba$posterior[, 2]
    
    
    #score <- ifelse(data_test_predict$class == "0", 1-score, score)
    print(score)
